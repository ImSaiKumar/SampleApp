...
body.on('click','.act-clear-search',function(){
  ...
  return false;
    ...
    //YorkTest-NM
      ...
      function getMealTrackerDetails(){
      ...
      function getExterciseTrackerDetails(){
        var etrackerDetails = {};
        if(connect.connected()){
     ...
     // body.on('click','.direct-menu',function(){
               ...
               // if(connect.connected())
                {
                  ...
                  api.invoke('addMealTrackerDetails',consumptionDetails,function(response){
                    if(response.status == '201'){
                      alert('Meal Tracker Created Successfully');
                    }  
                  });
                }
                //else{
      ...
      body.on('click','.ytLunch',function(){
                ...
                if(connect.connected()){
                  ...
                  api.invoke('addMealTrackerDetails',consumptionDetails,function(response){  
                    if(response.status == '201'){
                      alert('Meal Tracker for Dinner Created Successfully');
                    } 
                  });
                }else{
                    msg.show('loading from cache',2000);
      ...
      body.on('click','.ytSnacksPM',function(){
                ...
                if(connect.connected()){
                  ...
                  api.invoke('addMealTrackerDetails',consumptionDetails,function(response){  
                    if(response.status == '201'){
                      alert('Meal Tracker for Snacks Created Successfully');
                    }
                  });
                }else{
                    msg.show('loading from cache',2000);
      ...
      body.on('click','.ytExerciseTracker',function(){
                ...
                if(connect.connected()){
                  ...
                  api.invoke('addExerciseTrackerDetails',exerciseDetails, function(response){  
                    if(response.status == '201'){
                      alert('Exercise Tracker Created Successfully');
                    } 
                  });
                }else{
                  msg.show('loading from cache',2000);
  ...
  body.on('click','.act-menu',function(){
    ...
    if(pageName == '#meal-tracker'){
      getMealTrackerDetails();
    }else if(pageName == '#Exercise-tracker'){
      getExterciseTrackerDetails();
    }
